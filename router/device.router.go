...
func Device(app *fiber.App) {
	group := app.Group("/device", jwtware.New(jwtware.Config{
	...
	group.Get("", handler.ReadDevices)
	group.Post("", handler.CreateDevice)
	group.Put("", handler.UpdateDevice)
	group.Delete("", handler.DeleteDevice)
	group.Get("/find", handler.FindDevice)
}
